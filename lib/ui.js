...
/** 机票订单状态 → { text, cls }
 *  注意：实际接口返回的 status 可能是字符串状态码（如 "1"），
 *  也可能有 status_text 字段直接给中文。优先用 statusText 参数。
 */
export function flightStatus(status, statusText) {
  if (statusText) return { text: statusText, cls: flightStatusCls(status, statusText) };
  const map = {
    ...
    refunding: { text: "退款中", cls: "status-paid" },
    failed: { text: "出票失败，请联系客服", cls: "status-warn" },
    // 兼容字符串状态码（实测）
    "1": { text: "待付款", cls: "status-warn" },
    "2": { text: "已付款，正在出机票", cls: "status-paid" },
    "3": { text: "已出机票", cls: "status-success" },
    "4": { text: "出票失败，请联系客服", cls: "status-warn" },
    "5": { text: "已取消", cls: "status-cancel" },
  };
  return map[status] || { text: status || "未知", cls: "status-cancel" };
}
function flightStatusCls(status, statusText) {
  const t = (statusText || "") + "";
  if (/待支付|待付/.test(t)) return "status-warn";
  if (/已出|成功|已入/.test(t)) return "status-success";
  if (/已取消|已退/.test(t)) return "status-cancel";
  if (/失败|异常/.test(t)) return "status-warn";
  if (/已付|出票中|确认中/.test(t)) return "status-paid";
  return "status-cancel";
}

/** 酒店订单状态 → { text, cls }
 *  注意：酒店订单列表项不返回中文 status_text，只有英文 status / hotel_status。
 *  需要把所有英文状态码映射成中文。
 */
export function hotelStatus(status, statusText) {
  if (statusText) return { text: statusText, cls: flightStatusCls(status, statusText) };
  const map = {
    ...
    // pay_status 可能取值
    unpaid: { text: "待付款", cls: "status-warn" },
  };
  return map[status] || { text: status || "未知", cls: "status-cancel" };
}
